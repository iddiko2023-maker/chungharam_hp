    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {data.counts.map(([label, count], index) => {
          ...
          return (
          <div key={label} className="rounded-[24px] bg-white/84 p-5 ring-1 ring-[#DDE7E7]/65">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-bold text-[#64748B]">{label}</p>
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#E7F6F6] text-[#155F70]">
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1fr_1fr]">
        <section className="rounded-[26px] bg-white/84 p-6 ring-1 ring-[#DDE7E7]/65">
          ...
          <div className="mt-4 grid gap-3">
            {data.inquiries.map((item) => (
              <div key={item.id} className="rounded-[20px] bg-[#F8FAFB] p-4">
                <div className="flex items-center justify-between gap-3">
            ...
            {data.inquiries.length === 0 ? <p className="rounded-[20px] bg-[#F8FAFB] p-4 text-[#64748B]">표시할 신규 문의가 없습니다.</p> : null}
          </div>
        </section>
        <section className="rounded-[26px] bg-white/84 p-6 ring-1 ring-[#DDE7E7]/65">
          <div className="flex items-center justify-between gap-3 border-b border-[#EEF2F2] pb-4">
          ...
          <div className="mt-4 grid gap-3">
            ...
            {data.donations.length === 0 ? <p className="rounded-[20px] bg-[#F8FAFB] p-4 text-[#64748B]">표시할 신규 후원 신청이 없습니다.</p> : null}
          </div>
        </section>
      </div>
      <section className="mt-8 rounded-[28px] bg-[#155F70] p-6 text-white shadow-[0_16px_38px_rgba(21,95,112,0.14)]">
